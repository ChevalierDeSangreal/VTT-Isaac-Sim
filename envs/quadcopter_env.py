...
from omni.isaac.lab.sim import SimulationCfg, RenderCfg
from omni.isaac.lab.sim.spawners.materials import PreviewSurfaceCfg
...
from myutils import quaternion_to_matrix, matrix_to_euler_angles, euler_angles_to_matrix, matrix_to_quaternion, rand_circle_point, circular_motion
...
class QuadcopterEnvCfg(DirectRLEnvCfg):
	...
	target_mode = "line" # "line" "circle" "2circle" "test height"
	camera_pos = 'tdv' # 'fpv' 'tdv'(Top-Down View) 'tpv'
	fpv = (camera_pos == 'fpv') # if true, the camera will be placed to robot_position, otherwise it will be quadrotor
	...
	wall: RigidObjectCfg = RigidObjectCfg(
		...
		spawn=sim_utils.UsdFileCfg(
			usd_path="/home/wangzimo/VTT/IsaacLab/source/VTT/assets/Collected_Factory/MFactory.usd",
			rigid_props=sim_utils.RigidBodyPropertiesCfg(),
			copy_from_source=False,
	...
	if camera_pos == 'fpv':
		...
		tiled_camera: TiledCameraCfg = TiledCameraCfg(
			...
			offset=TiledCameraCfg.OffsetCfg(pos=(0, 0, 7.5), rot=(0.707, 0, 0.707, 0), convention="world"),
			data_types=["rgb", "distance_to_camera", "semantic_segmentation"],
			spawn=sim_utils.PinholeCameraCfg(
				focal_length=24.0, focus_distance=400.0, horizontal_aperture=75.6, vertical_aperture=75.38, clipping_range=(0.1, 50.0)
			), # 70H * 50V
			width=1224,
			height=1224,
			colorize_semantic_segmentation=True,
...
class QuadcopterEnv(DirectRLEnv):
	...
	def __init__(self, cfg: QuadcopterEnvCfg, render_mode: str | None = None, **kwargs):
		...
		if self.cfg.target_mode == "line":
			self.tar_acc_norm = 2
			self.tar_acc_intervel = 80 # How many time steps will acceleration change once
			self.tar_acc = torch.zeros((self._num_envs, 2), dtype=torch.float, device=self.device)
		elif self.cfg.target_mode == "circle":
			self.tar_center = torch.zeros((self._num_envs, 2), dtype=torch.float, device=self.device)
			self.tar_center[:, 0] = 1
			self.tar_center += self.scene.env_origins[:, :2]
			self.tar_r = 2
			self.tar_speed = 2
		elif self.cfg.target_mode == "2circle":
			self.now_circle = 0

			self.tar_center1 = torch.zeros((self._num_envs, 2), dtype=torch.float, device=self.device)
			self.tar_center1[:, 0] = 1
			self.tar_center1 += self.scene.env_origins[:, :2]
			self.tar_center2 = torch.zeros((self._num_envs, 2), dtype=torch.float, device=self.device)
			self.tar_center2[:, 0] = -3
			self.tar_center2 += self.scene.env_origins[:, :2]
			self.tar_center = [self.tar_center1, self.tar_center2]

			self.tar_r = 2

			self.tar_speed1 = 2
			self.tar_speed2 = -2
			self.tar_speed = [self.tar_speed1, self.tar_speed2]

			self.count_step += int(math.pi / self.cfg.sim.dt)
	...
	def _apply_action(self, action):
		...
		if self.cfg.target_mode == "line":
			...
			root_state = self._target.data.root_state_w.clone()
			new_pos = circular_motion(centers=self.tar_center,
									   radius=self.tar_r, 
									   current_positions=root_state[:, :2], 
									   time_interval=self.cfg.sim.dt, 
									   speed=self.tar_speed)
			root_state[:, :2] = new_pos
			root_state[:, 2] = 3
			root_state[:, 3] = 1
			root_state[:, 4:] = 0
			self._target.write_root_state_to_sim(root_state)
			...
			if (self.count_step[0] * self.cfg.sim.dt > (2 * math.pi)):
				self.now_circle = 1 - self.now_circle
				self.count_step *= 0


			root_state = self._target.data.root_state_w.clone()
			new_pos = circular_motion(centers=self.tar_center[self.now_circle], 
									  radius=self.tar_r, 
									  current_positions=root_state[:, :2], 
									  time_interval=self.cfg.sim.dt, 
									  speed=self.tar_speed[self.now_circle])
			root_state[:, :2] = new_pos
			root_state[:, 2] = 3
			root_state[:, 3] = 1
			root_state[:, 4:] = 0
		...
		elif self.cfg.target_mode == "test height":
			root_state = self._target.data.root_state_w.clone()
			# print(self.count_step[0])
			root_state[:, 2] = 3
			if self.count_step[0] < 50:
				# set linearvels
				root_state[:, 7] = 1
				root_state[:, 8] = 0
				root_state[:, 9] = 0
	...
	def reset_idxs(self, env_ids: torch.Tensor | None):
		...
		if self.cfg.target_mode == "2circle":
			self.count_step[env_ids] = int(math.pi / self.cfg.sim.dt)
		...
		# if self.cfg.target_mode == "test height":
	...
	def step(self, action: torch.Tensor) -> VecEnvStepReturn:
		...
		for _ in range(self.cfg.decimation):
			...
			if self.cfg.anno:
				...
				self.draw_drone_axes()

				# state = self._robot.data.root_state_w.clone()
				# batch_size = state.shape[0]
				# # 分解状态参数
				# positions = state[:, 0:3]          # 位置 (x, y, z)
				...
				# self.draw.draw_lines([start], [end], [(0, 1, 0, 1)], [3])
	...
	def save_image(self, typ='rgb', path='/home/wangzimo/VTT/IsaacLab/source/VTT/camera_output/frames/', name='tmp.png', idx=0):
	...
	def draw_drone_axes(self):
		...
		# === 无人机数据计算 ===
		# 姿态（X轴方向）
		robot_pos = robot_state[:, 0:3]
		robot_rot = quaternion_to_matrix(robot_state[:, 3:7])
		robot_x_dir = robot_rot[:, :, 0]  # 第一列为X轴
		robot_end_orient = robot_pos + robot_x_dir * 2  # 姿态线终点

		# 速度方向
		robot_vel = robot_state[:, 7:10]
		vel_norm = torch.norm(robot_vel, dim=1, keepdim=True)
		robot_vel_dir = robot_vel / torch.where(vel_norm > 1e-6, vel_norm, torch.ones_like(vel_norm))
		robot_end_vel = robot_pos + robot_vel_dir * 2

		# 加速度方向
		robot_acc = robot_state[:, 10:13]
		acc_norm = torch.norm(robot_acc, dim=1, keepdim=True)
		robot_acc_dir = robot_acc / torch.where(acc_norm > 1e-6, acc_norm, torch.ones_like(acc_norm))
		robot_end_acc = robot_pos + robot_acc_dir * 2

		# === 目标物体数据计算 ===
		target_pos = target_state[:, 0:3]
		
		# 目标速度方向
		target_vel = target_state[:, 7:10]
		t_vel_norm = torch.norm(target_vel, dim=1, keepdim=True)
		target_vel_dir = target_vel / torch.where(t_vel_norm > 1e-6, t_vel_norm, torch.ones_like(t_vel_norm))
		target_end_vel = target_pos + target_vel_dir * 2

		# 目标加速度方向
		target_acc = target_state[:, 10:13]
		t_acc_norm = torch.norm(target_acc, dim=1, keepdim=True)
		target_acc_dir = target_acc / torch.where(t_acc_norm > 1e-6, t_acc_norm, torch.ones_like(t_acc_norm))
		target_end_acc = target_pos + target_acc_dir * 2

		# === 合并所有可视化数据 ===
		# 起点列表（姿态/速度/加速度 x2）
		starts = torch.cat([
			robot_pos, robot_pos, robot_pos,  # 无人机三项
			# target_pos, target_pos            # 目标两项（无姿态）
		])

		# 终点列表（顺序需与起点对应）
		ends = torch.cat([
			robot_end_orient, robot_end_vel, robot_end_acc,
			# target_end_vel, target_end_acc
		])

		# 颜色配置（RGBA）
		colors = (
			[(0, 1, 0, 1)] * batch_size +    # 无人机姿态-绿色
			[(1, 0, 0, 1)] * batch_size +    # 速度-红色 
			[(0, 0.5, 1, 1)] * batch_size   # 加速度-天蓝
			#  + [(1, 0, 0, 1)] * batch_size +    # 目标速度-红色
			# [(0, 0.5, 1, 1)] * batch_size    # 目标加速度-天蓝
		)

		# 线条粗细统一为3
		# sizes = [3] * (5 * batch_size)  # 总线条数：3(robot)+2(target)
		sizes = [3] * (3 * batch_size)

		# 执行绘制
		self.draw.draw_lines(
			starts.cpu().numpy().tolist(),
			ends.cpu().numpy().tolist(),
			colors,
			sizes
		)